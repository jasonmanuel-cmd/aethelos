...
export class XDateService {
  ...
  async dispatchDailyAutomations() {
    logger.info('Running daily X-Date automation dispatch...');
    ...
    if (!targetLeads.length) {
      logger.info('No X-Date automations to dispatch today');
      return;
    }
    ...
    logger.info(`Dispatched ${targetLeads.length} X-Date automation sequences`);
  }
